...
rl.question('Where do you want the downloader to save all your anime? (Provide a root folder, not an anime-specific one)', (answer) => {
...
function getAnime() {
	var iterator = 0;
	rl.question('Please give your kissanime bookmarklist url or anime name (start anime names with "anime:"): ', (answer) => {
		if (answer.substring(0, 6).toLowerCase() == "anime:") {
			subProcess = child_process.spawn('node', ['singleAnime.js', answer.substring(6), basePath, (process.argv[3] || 5)]);
